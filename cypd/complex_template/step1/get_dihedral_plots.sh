#! /bin/bash
#SBATCH --job-name=dihedral_plots
...
if [ ! -d "/path/to/dir" ]; then
    mkdir dihedral_switch_plots
fi

echo "Submitting ${ID}"
echo "Job directory: ${SLURM_SUBMIT_DIR}"

declare -a ReplicateArray=("1" "2" "3")

for outer_dir in ${ReplicateArray[@]}; do
	for lambda_dir in ${outer_dir}/*/ ; do
		python dihedral_switch.py --replicate-lambda-dir ${lambda_dir}Production_MD 
	done
done
