...
discard = 1000
#upper time (ps) for performing dG estimate
upper = 10000
...
for r in repeats:
    try:
        ...
        u_nks_all = []
        for i in range(0, lambdanr):
            folder = '%s/lambda_%i/Production_MD'%(path,i)
        ...
        for time in timeslices:
            ...
            for i in range(0, lambdanr):
                ...
                statinef  = statistical_inefficiency(u_nk[u_nk.columns[i]], fast=False)
                ...
                if len(indices) >= 1000:
                    ...
                    #extract 1000 data points
                    skip = int(number/1000)
                    #Slice data to obtain 1000 samples
            ...
            endpoint_diff = 0.596 * mbar.delta_f_.loc[[(0.00,0.00)], [(1.00,1.00)]]
            dg_diff = round(float(endpoint_diff.iloc[0][(1.00,1.00)]),3)
            dGs.append(dg_diff)
            uncertainty = 0.596 * mbar.d_delta_f_.loc[[(0.00,0.00)], [(1.00,1.00)]]
            uncertainty = round(float(uncertainty.iloc[0][(1.00,1.00)]),3)
            ...
            if time == timeslices[-1]:
                ...
                ax.figure.savefig('%s/O_MBAR.pdf'%path, bbox_inches='tight', pad_inches=0.0)
        print(dGs)
