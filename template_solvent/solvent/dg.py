...
def test(path, lambdanr):
    ...
    for i in range(0, lambdanr):
        count = 0
        start = False
        xvg = '%s/lambda_%i/Production_MD/prod.xvg' %(path,i)
        u_nk = open(xvg,'r')
        for x in u_nk:
...
# Initialize a dictionary to store pairwise ΔG values for each repeat
all_pairwise_dG = {}
    ...
    try:
        ...
        for time in timeslices:
            ...
            if time == timeslices[-1]:
                ...
                mbar_pymbar = pymbar.mbar.MBAR(u_kn, N_k,solver_protocol="robust")
                results = mbar_pymbar.compute_free_energy_differences()
                dG = results['Delta_f']
                pairwise_dG = [dG[i, i+1] for i in range(len(N_k)-1)]
                pairwise_dG_rounded = [round(value, 3) for value in pairwise_dG]
                print("Pairwise ΔG:", pairwise_dG_rounded)
                # Store the values in a dictionary with the repeat number as the key
                all_pairwise_dG[f"Repeat {r}"] = pairwise_dG_rounded
                overlap_matrix = mbar_pymbar.compute_overlap()
                overlap_matrix = overlap_matrix['matrix']
                ax = plot_mbar_overlap_matrix(overlap_matrix)
                ax.figure.savefig('%i/O_MBAR.pdf'%(r), bbox_inches='tight', pad_inches=0.0) 
                plt.close(ax.figure)   
        print(dGs)
        print(dG_error)
...
# Save all collected pairwise ΔG values to a CSV file
csv_filename = "pairwise_dG.csv"

# Ensure the data is properly formatted for writing
max_length = max(len(v) for v in all_pairwise_dG.values())  # Get the max number of λ-pairs

# Pad shorter lists with NaNs for alignment
for key in all_pairwise_dG:
    while len(all_pairwise_dG[key]) < max_length:
        all_pairwise_dG[key].append("NaN")  # Use "NaN" to indicate missing values

# Write to CSV
with open(csv_filename, mode="w", newline="") as file:
    writer = csv.writer(file)

    # Write header with repeat numbers
    writer.writerow(all_pairwise_dG.keys())

    # Write rows by transposing the dictionary values
    writer.writerows(zip(*all_pairwise_dG.values()))
